...
interface EnumSettingMetadata {
  type: 'enum';
  options: Record<string, number>;
}

type SettingMetadata = EnumSettingMetadata;
...
export const SettingsModal = ({ isOpen, onClose }: SettingsModalProps) => {
  ...
  const getEnumOptionLabel = (settingKey: string, optionKey: string) => {
    const translationKey = `serialLog.configureSettings.options.${settingKey}.${optionKey}`;
    return t(translationKey, { defaultValue: humanizeCamelCase(optionKey) });
  };

  const getEnumOptions = (settingKey: string) => {
    const fieldMetadata = metadata[settingKey];

    if (!fieldMetadata) {
      return [];
    }

    return Object.entries(fieldMetadata.options).map(([optionKey, optionValue]) => ({
      key: optionKey,
      value: optionValue,
      label: getEnumOptionLabel(settingKey, optionKey),
    }));
  };

  const getDisplayValue = (key: string, value: SettingValue) => {
    const enumOptions = getEnumOptions(key);

    if (enumOptions.length > 0 && typeof value === 'number') {
        ...
        <div className="space-y-4">
          {defaults &&
            Object.keys(defaults).map(key => {
              ...
              const isEnum = !!fieldMetadata;
              const enumOptions = getEnumOptions(key);
              const value = settings[key];

              return (
                <div key={key}>
                  {/* TODO: Localize non-enum setting labels and boolean state text once translation keys exist for the rest of the settings schema. */}
                  <label
                    htmlFor={`setting-${key}`}
                  ...
                  <p className="mt-1 text-xs theme-muted">
                    Default: {getDisplayValue(key, defaults[key])}
                  </p>
                </div>
              );
            })}
