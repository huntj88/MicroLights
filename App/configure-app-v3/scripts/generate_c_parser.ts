...
const incDir = path.join(repoRoot, 'BulbChipSTM32C071FBPx/Core/Inc/json');
const srcDir = path.join(repoRoot, 'BulbChipSTM32C071FBPx/Core/Src/json');

const incPath = path.join(incDir, 'mode_parser.h');
const srcPath = path.join(srcDir, 'mode_parser.c');

// Ensure directories exist (optional, but good practice if we are generating into a structure that might be clean)
// However, usually Core/Inc and Core/Src exist in STM32 projects.

try {
  fs.writeFileSync(incPath, generateHeader());
  console.log(`Generated ${incPath}`);
} catch (e: unknown) {
  console.error(`Failed to write to ${incPath}:`, e);
  console.log('Falling back to local write for mode_parser.h');
  fs.writeFileSync('mode_parser.h', generateHeader());
}

try {
  fs.writeFileSync(srcPath, generateSource());
  console.log(`Generated ${srcPath}`);
} catch (e: unknown) {
  console.error(`Failed to write to ${srcPath}:`, e);
