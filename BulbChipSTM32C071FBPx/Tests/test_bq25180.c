...
void rgbShowNotCharging(RGBLed *led) { rgbNotChargingCalled = true; }
void rgbShowConstantCurrentCharging(RGBLed *led) { rgbConstantCurrentCalled = true; }
void rgbShowConstantVoltageCharging(RGBLed *led) { rgbConstantVoltageCalled = true; }
void rgbShowDoneCharging(RGBLed *led) { rgbDoneChargingCalled = true; }
...
void setUp(void) {
    ...
    bq25180Init(&charger, mock_readRegister, mock_writeRegister, 0x6A, mock_writeToUsbSerial, &mockLed);
