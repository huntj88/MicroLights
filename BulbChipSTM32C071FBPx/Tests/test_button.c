...
void rgbShowNoColor(RGBLed *led) { rgbNoColorCalled = true; }
void rgbShowLocked(RGBLed *led) { rgbLockedCalled = true; }
void rgbShowShutdown(RGBLed *led) { rgbShutdownCalled = true; }
void rgbShowSuccess(RGBLed *led) { rgbSuccessCalled = true; }
...
void setUp(void) {
    ...
    buttonInit(&button, mock_readButtonPin, mock_startButtonTimer, mock_stopButtonTimer, &mockCaseLed);
}
