...
bool BQ25180::write(uint16_t reg_addr, uint8_t flags)
{
...
void dumpStat1Register(Adafruit_I2CDevice *i2c)
{
    ...
    uint8_t TS_STAT_1_0 = (flags & 0b00011000) >> 3; // Bits 4-3 for TS Status (2 bits)
...
void dumpVbatCtrlRegister(Adafruit_I2CDevice *i2c)
{
    ...
    uint8_t VBATREG_CODE = flags & 0b01111111; // VBATREG_6:0 (Bits 6-0)
...
void dumpIchgCtrlRegister(Adafruit_I2CDevice *i2c)
{
    ...
    uint8_t ICHGCODE = flags & 0b01111111; // ICHG_6:0 (Bits 6-0)
...
void dumpChargeCtrl0Register(Adafruit_I2CDevice *i2c)
{
    ...
    uint8_t ITERM = (flags >> 4) & 0b11;  // Bits 5-4
    uint8_t VINDPM = (flags >> 2) & 0b11; // Bits 3-2
    uint8_t THERM_REG = flags & 0b11;     // Bits 1-0
...
void dumpChargeCtrl1Register(Adafruit_I2CDevice *i2c)
{
    ...
    // Extract each bit field
    uint8_t IBAT_OCP = (flags >> 6) & 0b11;          // Bits 7-6
    uint8_t BUVLO = (flags >> 3) & 0b111;            // Bits 5-3
...
void dumpICCtrlRegister(Adafruit_I2CDevice *i2c)
{
    ...
    uint8_t SAFETY_TIMER = (flags >> 2) & 0b11; // Bits 3-2
    uint8_t WATCHDOG_SEL = flags & 0b11;        // Bits 1-0
...
void dumpTMR_ILIMRegister(Adafruit_I2CDevice *i2c)
{
    ...
    // Extract each bit field
    uint8_t MR_LPRESS = (flags >> 6) & 0b11;  // Bits 7-6
    bool MR_RESET_VIN = (flags >> 5) & 0b1; // Bit 5
    uint8_t AUTOWAKE = (flags >> 3) & 0b11;   // Bits 4-3
    uint8_t ILIM = flags & 0b111;             // Bits 2-0
...
void dumpSHIP_RSTRegister(Adafruit_I2CDevice *i2c)
{
    ...
    uint8_t EN_RST_SHIP = (flags >> 5) & 0b11;      // Bits 6-5
    uint8_t PB_LPRESS_ACTION = (flags >> 3) & 0b11; // Bits 4-3
...
void dumpSYS_REGRegister(Adafruit_I2CDevice *i2c)
{
    ...
    // Extract each bit field
    uint8_t SYS_REG_CTRL = (flags >> 5) & 0b111;     // Bits 7-5
    bool RESERVED = (flags >> 4) & 0b1;            // Bit 4
    uint8_t SYS_MODE = (flags >> 2) & 0b11;          // Bits 3-2
...
void dumpTS_CONTROLRegister(Adafruit_I2CDevice *i2c)
{
    ...
    // Extract each bit field
    uint8_t TS_HOT = (flags >> 6) & 0b11;  // Bits 7-6
    uint8_t TS_COLD = (flags >> 4) & 0b11; // Bits 5-4
...
void dumpMASK_IDRegister(Adafruit_I2CDevice *i2c)
{
    ...
    uint8_t deviceID = flags & 0b1111;         // Bits 3-0 (Device ID)
